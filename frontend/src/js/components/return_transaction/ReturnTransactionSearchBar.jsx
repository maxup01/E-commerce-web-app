...
function ReturnTransactionSearchBar(props) {
    ...
    const [returnCauses, setReturnCauses] = useState([]);
    ...
    useEffect(() => {
        ...
        axios.get('http://127.0.0.1:8080/all-return-causes')
            .then(response => {
                setReturnCauses(response.data);
            });
    });
    ...
    function handleReturnCauseChange(event) {

        props.returnCauseNameSetter(event.target.value !== "" ? event.target.value : null);
    }
    ...
    return (
        <div className={"grid grid-cols-6 grid-rows-2"} style={divStyle}>
            ...
            <select onChange={handleReturnCauseChange}
                    className={"bg-cyan-50 row-start-1 row-span-1 col-start-3 col-span-2 "}>
                {returnCauses.map(returnCause => (
                    <option key={returnCause} value={returnCause}>{returnCause}</option>
                ))}
            </select>
...
export default ReturnTransactionSearchBar;